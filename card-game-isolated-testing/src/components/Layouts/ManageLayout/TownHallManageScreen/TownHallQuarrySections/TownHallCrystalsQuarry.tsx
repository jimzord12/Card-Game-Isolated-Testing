...
import CrystalsQuarrySliderSection from "../../../QuarriesLayout/QuarrySliderSections/CrystalsQuarrySliderSection";

interface Props {
  handleGoBack: () => void;
}

const TownHallCrystalsQuarry = ({ handleGoBack }: Props) => {
  ...
  return (
    <div className="relative flex flex-col items-center gap-2 tablet:gap-4">
      <div className="absolute left-8 top-4 flex justify-center items-center gap-2 tablet:gap-4">
        <QuarryLevelIndicator level={quarryLevels.crystals} type="crystals" />
      ...
      <div className="absolute right-28 tablet:right-36 largeScreen:right-44 top-4 flex justify-center items-start gap-2 tablet:gap-4">
        <QuarryLevelUpBtn type="crystals" />
      </div>
      <CrystalsQuarrySliderSection />
    </div>
  );
};

export default TownHallCrystalsQuarry;
