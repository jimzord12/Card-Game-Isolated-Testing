...
const Townhall_LvlUp_NewStats = () => {
  const size = useGetLabelsSize();
  const gameVars = useGameVarsStore((state) => state);
  const townhallLevel = gameVars.townhallLevel;
  ...
  const currentCitizensLimit =
    defaultBuildingsConfig.townhallHousingLimitPerLevel[townhallLevel];
  const nextCitizensLimit =
    defaultBuildingsConfig.townhallHousingLimitPerLevel[
      (townhallLevel + 1) as Level
    ];

  const currentSpace =
    defaultBuildingsConfig.townhallAvailSpacePerLevel[townhallLevel];
  const nextSpace =
  ...
  return (
    <>
      {gameVars.townhallLevel === 5 ? (
        <h2 className="text-white text-3xl text-center">
          Townhall's Level is Maxed out! You can not level it up any further.
        </h2>
      ) : (
        <section
...
export default Townhall_LvlUp_NewStats;
